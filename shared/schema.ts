...
// Gmail Campaign Assignment - for assigning URLs to campaigns based on click quantity ranges
export const gmailCampaignAssignments = pgTable("gmail_campaign_assignments", {
  id: serial("id").primaryKey(),
  campaignId: integer("campaign_id").references(() => campaigns.id).notNull(),
  minClickQuantity: integer("min_click_quantity").default(1).notNull(), // Minimum click quantity for assignment
  maxClickQuantity: integer("max_click_quantity").default(1000000000).notNull(), // Maximum click quantity for assignment
  priority: integer("priority").default(1).notNull(), // Priority (lower = higher priority)
  active: boolean("active").default(true).notNull(), // Is this assignment active
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});

export const insertGmailCampaignAssignmentSchema = createInsertSchema(gmailCampaignAssignments).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export const updateGmailCampaignAssignmentSchema = z.object({
  campaignId: z.number().int().optional(),
  minClickQuantity: z.number().int().min(1).optional(),
  maxClickQuantity: z.number().int().min(1).optional(),
  priority: z.number().int().min(1).optional(),
  active: z.boolean().optional(),
});

export type GmailCampaignAssignment = typeof gmailCampaignAssignments.$inferSelect;
export type InsertGmailCampaignAssignment = z.infer<typeof insertGmailCampaignAssignmentSchema>;
export type UpdateGmailCampaignAssignment = z.infer<typeof updateGmailCampaignAssignmentSchema>;

// Relation between Gmail Campaign Assignments and Campaigns
export const gmailCampaignAssignmentsRelations = relations(gmailCampaignAssignments, ({ one }) => ({
  campaign: one(campaigns, {
    fields: [gmailCampaignAssignments.campaignId],
    references: [campaigns.id],
  }),
}));
