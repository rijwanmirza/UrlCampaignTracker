...
export default function StatsCards({ campaign }: StatsCardsProps) {
  ...
  // Calculate clicks per URL
  const clicksPerUrl = campaign.urls.length > 0 
    ? (campaign.totalClicks / campaign.urls.length).toFixed(1) 
    : "0";
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold">
            {campaign.urls.length - campaign.activeUrlCount}/{campaign.urls.length}
          </div>
          <div className="text-xs text-muted-foreground mt-1">
          ...
          <div className="mt-3 h-2 rounded-full bg-gray-100">
            <div
              className="h-full bg-primary rounded-full"
              style={{ width: `${100 - activeUrlPercent}%` }}
            />
          </div>
        </CardContent>
      </Card>
