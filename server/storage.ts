...
export class DatabaseStorage implements IStorage {
  ...
  async fullSystemCleanup(): Promise<{ campaignsDeleted: number, urlsDeleted: number, originalUrlRecordsDeleted: number, analyticsRecordsDeleted: number }> {
    try {
      ...
      try {
        ...
        console.log(`ℹ️ SYSTEM RESET: No analytics data found to count`);
      }
      
      console.log(`🧹 SYSTEM RESET: Starting complete database cleanup - Found ${allCampaigns.length} campaigns, ${totalUrls} URLs, ${originalRecordsCount} original URL records, and ${analyticsCount} analytics records to delete`);
      ...
      try {
        ...
        console.log(`✅ SYSTEM RESET: Deleted all analytics data (${analyticsCount} records)`);
      } catch (error) {
        console.log(`ℹ️ SYSTEM RESET: No analytics table found or nothing to delete`);
      }
      
      // 1. Delete all URLs (to handle foreign key constraints)
      ...
      try {
        ...
        try {
          ...
          console.log(`✅ SYSTEM RESET: Reset click analytics sequence to start from ID 1`);
        } catch (error) {
          // Ignore error if table doesn't exist
          console.log(`ℹ️ SYSTEM RESET: No click analytics sequence found to reset`);
        }
      ...
      console.log(`✅ SYSTEM RESET COMPLETED: Successfully deleted ${allCampaigns.length} campaigns, ${totalUrls} URLs, ${originalRecordsCount} original URL records, and ${analyticsCount} analytics records`);
  ...
  private async getTimeseriesData(
    ...
    try {
      // Determine time format and grouping based on filter
